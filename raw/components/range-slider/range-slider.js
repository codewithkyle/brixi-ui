import{html as i,render as o}from"./lit-html.js";import{unsafeHTML as n}from"./unsafe-html.js";import l from"./env.js";import{InputBase as r}from"./input-base.js";import{calcPercent as m}from"./numpy.js";import{UUID as d}from"./uuid.js";l.css(["range-slider"]);class s extends r{constructor(){super();this.handleChange=e=>{e.stopImmediatePropagation()};this.handleInput=e=>{e.stopImmediatePropagation();const a=e.currentTarget;let t=parseInt(a.value);isNaN(t)&&(t=this.model.min),t<this.model.min?t=this.model.max:t>this.model.max&&(t=this.model.max),this.renderFill(t),this.set({value:t}),this.dispatchEvent(new CustomEvent("change",{detail:{value:t,name:this.model.name}}))};this.handleBlur=e=>{e.stopImmediatePropagation();const a=e.currentTarget;let t=parseInt(a.value);isNaN(t)&&(t=this.model.min),t<this.model.min?t=this.model.max:t>this.model.max&&(t=this.model.max),this.renderFill(t),this.set({value:t}),this.dispatchEvent(new CustomEvent("blur",{detail:{value:t,name:this.model.name}}))};this.handleFocus=e=>{e.stopImmediatePropagation(),this.dispatchEvent(new CustomEvent("focus",{detail:{value:this.model.value,name:this.model.name}}))};this.handleIconClick=()=>{let e=0;this.model.value===this.model.min?e=this.model.max:e=this.model.min,this.renderFill(e),this.set({value:e}),this.dispatchEvent(new CustomEvent("change",{detail:{value:e,name:this.model.name}}))};this.inputId=d(),this.model={manual:!1,label:"",name:"",instructions:"",readOnly:!1,required:!1,disabled:!1,error:"",icon:"",minIcon:null,maxIcon:null,value:null,min:0,max:9999,step:1,autofocus:!1}}static get observedAttributes(){return["data-label","data-name","data-instructions","data-icon","data-read-only","data-required","data-manual","data-disabled","data-value","data-min","data-max","data-step","data-autofocus","data-min-icon","data-max-icon"]}reset(){this.set({value:this.model.min})}validate(){return!0}renderCopy(){let e="";return this.state==="IDLING"&&this.model.instructions?e=i`<p>${n(this.model.instructions)}</p>`:this.state==="ERROR"&&this.model.error&&(e=i`<p class="font-danger-700">${this.model.error}</p>`),e}renderLabel(){let e="";return this.model.label?.length&&(e=i`<label for="${this.inputId}">${n(this.model.label)}</label>`),e}renderManualInput(){let e="";return this.model.manual&&(e=i`
                <input
        ...
        `;o(e,this)}}l.bind("range-slider",s);export{s as default};
