...
import env from "~controllers/env";
import { parseDataset } from "~utils/general";
import { default as ${extend}, I${extend}, ${extend}Settings } from "~components${
		path ? `/${path}/` : "/"
	}${extend.toKebabCase()}";
...
(async () => {
	...
	const script = extendClassName
		? renderTypescriptExtended(
				name,
				tagName,
				extendClassName,
				extendedClassPath
		  )
		: renderTypescript(name, tagName);
	await fs.promises.writeFile(
		path.join(newDir, `${name.toKebabCase()}.ts`),
		script,
		{ encoding: "utf-8" }
	);
	await fs.promises.writeFile(
		path.join(newDir, `${name.toKebabCase()}.scss`),
		renderSass(tagName),
		{ encoding: "utf-8" }
	);
})();
