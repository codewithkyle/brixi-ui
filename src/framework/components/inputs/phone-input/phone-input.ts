...
export default class PhoneInput extends Input {
    ...
    override validate(input:HTMLInputElement = null, clearOnly = false): boolean {
        ...
        let isValid = true;
        const PhoneNumberCheck = new RegExp(/^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/gmi);
    ...
    override render(){
        ...
        const view = html`
            ...
            <input-container>
                ${this.renderIcon()}
                <input @focus=${this.handleFocus} @input=${this.handleInput} @blur=${this.handleBlur} inputmode="tel" type="tel" id="${id}" .value=${this.model.value} placeholder=${this.model.placeholder} name=${this.model.name} autocapitalize=${this.model.autocapitalize} autocomplete="${this.model.autocomplete}" ?required=${this.model.required} ?disalbed=${this.model.disabled} />
            </input-container>
        `;
