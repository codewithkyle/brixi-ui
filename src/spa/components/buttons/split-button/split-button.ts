...
export default class SplitButton extends Component<ISplitButton> {
    ...
    private renderPrimaryButton() {
        return html`
            <button class="base" sfx="button" type=${this.model.type} @click=${this.handlePrimaryClick}>${this.renderIcon(this.model.icon)} ${this.renderLabel(this.model.label)}</button>
        `;
    }
