import{html as l,render as r}from"./lit-html.js";import{unsafeHTML as i}from"./unsafe-html.js";import a from"./env.js";import{noop as s}from"./general.js";import{InputBase as m}from"./input-base.js";import{calcPercent as u}from"./numpy.js";class o extends m{constructor(e){super(e);this.handleInput=e=>{const n=e.currentTarget;let t=parseInt(n.value);isNaN(t)&&(t=this.model.min),t<this.model.min?t=this.model.max:t>this.model.max&&(t=this.model.max),this.renderFill(t),this.set({value:t},!0),this.model.callbacks?.onInput&&typeof this.model.callbacks?.onInput=="function"&&this.model.callbacks?.onInput(t)};this.handleBlur=e=>{const n=e.currentTarget;let t=parseInt(n.value);isNaN(t)&&(t=this.model.min),t<this.model.min?t=this.model.max:t>this.model.max&&(t=this.model.max),this.renderFill(t),this.set({value:t}),this.model.callbacks?.onBlur&&typeof this.model.callbacks?.onBlur=="function"&&this.model.callbacks?.onBlur(t)};this.handleFocus=()=>{this.model.callbacks?.onFocus&&typeof this.model.callbacks?.onFocus=="function"&&this.model.callbacks?.onFocus(this.model.value)};this.handleIconClick=()=>{let e=0;this.model.value===this.model.min?e=this.model.max:e=this.model.min,this.renderFill(e),this.set({value:e}),this.model.callbacks?.onInput&&typeof this.model.callbacks?.onInput=="function"&&this.model.callbacks?.onInput(e)};this.model={manual:!1,label:"",name:"",instructions:"",readOnly:!1,required:!1,disabled:!1,error:"",icon:"",minValueIcon:null,maxValueIcon:null,placeholder:"",value:e?.min,min:0,max:9999,step:1,css:"",class:"",callbacks:{onInput:s,onFocus:s,onBlur:s},attributes:{},autofocus:!1},a.css(["range-slider"]).then(()=>{this.set(e,!0),this.render(),this.className=`js-input ${this.model.class}`,this.style.cssText=this.model.css,this.renderFill(this.model.value)})}reset(){this.set({value:this.model.min})}validate(){return!0}renderCopy(){let e="";return this.state==="IDLING"&&this.model.instructions?e=l`<p>${i(this.model.instructions)}</p>`:this.state==="ERROR"&&this.model.error&&(e=l`<p class="font-danger-700">${this.model.error}</p>`),e}renderLabel(e){let n="";return this.model.label?.length&&(n=l`<label for="${e}">${i(this.model.label)}</label>`),n}renderManualInput(){let e="";return this.model.manual&&(e=l`
                <input
