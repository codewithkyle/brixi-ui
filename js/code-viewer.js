import l from"./supercomponent.js";import{html as i,render as n}from"./lit-html.js";class h extends l{constructor(e){super();this.switchSource=e=>{const s=e.currentTarget.dataset.ext;console.log(s),this.update({activeExt:s})};this.copyToClipboard=e=>{let t;const s=e.currentTarget;for(let o=0;o<this.model.sourceCode.length;o++)if(this.model.sourceCode[o].ext===this.model.activeExt){t=this.model.sourceCode[o];break}navigator.clipboard.writeText(t.raw).then(()=>{s.classList.add("is-success"),setTimeout(()=>{s.classList.remove("is-success")},3e3)})};this.component=e,this.state="LOADING",this.stateMachine={LOADING:{SUCCESS:"IDLING"}},this.model={sourceCode:[],activeExt:null},this.render(),this.fetchFiles()}async fetchFiles(){const e=["readme.md",`${this.component}.html`,`${this.component}.ts`,`${this.component}.scss`,`${this.component}.css`,`${this.component}.js`];let t=0;const s={...this.model};for(let o=0;o<e.length;o++)new Promise(async c=>{const r=await fetch(`/raw/components/${this.component}/${e[o]}`);if(r.ok){const a=await r.text();s.sourceCode.push({ext:e[o].match(/\.[0-9a-z]+$/)?.[0]??"plaintext",raw:a})}c()}).then(()=>{t++,e.length===t&&(s.activeExt=s.sourceCode[0].ext,s.sourceCode.sort((c,r)=>c.ext>r.ext),this.update(s),this.trigger("SUCCESS"))})}render(){let e;this.state==="LOADING"?e=i`
