import a from"./supercomponent.js";import{message as i}from"./messaging.js";import{html as n,render as o}from"./lit-html.js";class s extends a{constructor(){super();this.navigate=t=>{const r=t.currentTarget.dataset.slug;this.update({active:r}),window.history.replaceState(null,null,`/${r}`),i({recipient:"view",data:r})};this.toggleGroup=t=>{t.currentTarget.classList.toggle("is-open")};this.handleMenuClick=t=>{document.body.classList.toggle("is-open")};this.state="LOADING",this.stateMachine={LOADING:{SUCCESS:"IDLING"}},this.model={navigation:[],active:null},this.render()}async fetchNavigation(){const e=await(await fetch("/navigation.json",{method:"GET",headers:new Headers({Accept:"application/json"})})).json();this.update({navigation:e}),this.trigger("SUCCESS")}renderLink(t){return n`
        ...
        `}render(){let t;switch(this.state){case"LOADING":t=n`
                ...
                `;break}o(t,this)}connected(){this.fetchNavigation();const t=location.pathname.replace(/^\//,"").trim();this.update({active:t})}}export{s as default};
