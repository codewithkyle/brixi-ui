import r from"./supercomponent.js";import{message as i}from"./messaging.js";import{html as n,render as s}from"./lit-html.js";class o extends r{constructor(){super();this.navigate=t=>{const a=t.currentTarget.dataset.slug;this.update({active:a}),window.history.replaceState(null,null,`/${a}`),i({recipient:"view",data:a})};this.toggleGroup=t=>{t.currentTarget.classList.toggle("is-open")};this.handleMenuClick=t=>{document.body.classList.toggle("is-open")};this.state="LOADING",this.stateMachine={LOADING:{SUCCESS:"IDLING"}},this.model={navigation:[],active:null},this.render()}async fetchNavigation(){const e=await(await fetch("/navigation.json",{method:"GET",headers:new Headers({Accept:"application/json"})})).json();this.update({navigation:e}),this.trigger("SUCCESS")}renderLink(t){return n`
        ...
        `}render(){let t;switch(this.state){case"LOADING":t=n`
                ...
                `;break}s(t,this)}connected(){this.fetchNavigation();const t=location.pathname.replace(/^\//,"").trim();this.update({active:t})}}export{o as default};
